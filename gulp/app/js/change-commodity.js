...
require(['config', 'comm'], function() {
	require(['vendor', 'fx_methods'], function() {
		// tab
		$('.vegetables-class').on('click', '.ovfs-item', function() {
		...
		// 删除单个商品
		$('.chosed-wrap').on('click', '.chose-close', function() {
		...
		// 清空商品
		$(document).on('click', '.js-empty', function() {
		...
		// 商品添加
		$('.bg-bottom').on('click', '.bot-right', function() {
			// ....
			setTimeout(function() {
				$('.commodity-content').trigger('apperBottom');
			}, 300);
