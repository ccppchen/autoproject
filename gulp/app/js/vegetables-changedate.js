...
var appenddate=function(givey,givem,gived,speciald,hasselect,clicknum,limitmon){
				...
				var datecontroller=function(year,month,today){
					...
					$(".choosed").remove();
					if(hasselect!=null&&hasselect!=undefined){//已选渲染
					...
					$(".time-body li").click(function(){ //点击事件
							...
							if(span.hasClass("font-color-ash2")||span.text()==''){
								...
								$(".time-body li span").removeClass("select");
								...
								/*var html='<div class="selected-label"><div class="label-body"><div class="close-selected"></div><div class="label-time"><p><label>'+selectmonth+'</label><span>·</span><label>'+d+'</label></p><p><span class="send-week">周'+week+'配送</span></p></div></div></div>';
									...
									if(spm+1==showm){
										for (var j=0;j<firstweek;j++) {
											if(parseInt($(".time-body li span").eq(j).text())==spd){
												$(".time-body li span").eq(j).removeClass("select");
											}
										}
									}
									$(this).remove();
...
var limitmon=[20160727,20160929];
appenddate(null,null,null,null,choosed,8,limitmon);
$(".setmeal-alert").hide();
